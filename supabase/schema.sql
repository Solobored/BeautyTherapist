...
CREATE POLICY "Sellers can read their own brand" ON brands
  FOR SELECT USING (auth.uid() = owner_id);

CREATE POLICY "Sellers can update their own brand" ON brands
  FOR UPDATE USING (auth.uid() = owner_id);

CREATE POLICY "Sellers can create brands" ON brands
...
CREATE POLICY "Sellers can see their own products" ON products
  FOR SELECT USING (
    EXISTS (
...
-- Product images RLS
CREATE POLICY "Anyone can read product images of active products" ON product_images
  FOR SELECT USING (
    EXISTS (
...
-- Addresses RLS
CREATE POLICY "Users can read their own addresses" ON addresses
...
CREATE POLICY "Users can read their own orders" ON orders
  FOR SELECT USING (
    auth.uid() = user_id OR 
    (auth.uid()::text = buyer_email) OR 
    EXISTS (
      SELECT 1 FROM profiles WHERE profiles.id = auth.uid() AND user_type = 'seller'
    )
  );

CREATE POLICY "Anyone can insert orders (guests allowed)" ON orders
...
CREATE POLICY "Users can insert to their wishlist" ON wishlist
  FOR INSERT WITH CHECK (auth.uid() = user_id);
